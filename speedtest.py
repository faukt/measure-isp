...
def parse_args():
    ...
    parser.add_argument('--logfile', default=False, action='store_true',
                        help='Write to logfile in addition to default logging. Logfile stored in "./logs/[DATE].log"')
...
def filePrinter(string, **kwargs):
    try:
        os.mkdir('./logs/')
    except Exception as Ex:
        if Ex is None or hasattr(Ex, 'winerror') == False or Ex.winerror is None or Ex.winerror != 183:
            printer('Error IO operation! Ex: %s' % str(Ex), useFile=False)
            return

    try:
        filename = './logs/' + datetime.datetime.now().strftime("%Y-%m-%d")
        filename += '.log'
        printer('filename: %s' % filename, useFile=False)

        with open(filename, 'a+') as filePointer:
            filePointer.write(string+'\n')

    except Exception as Ex:
        printer('Error IO operation! Ex: %s' % str(Ex), useFile=False, quiet=False)

def printer(string, quiet=False, debug=False, error=False, useFile=True, **kwargs):
    """Helper function print a string with various features"""

    if debug and not DEBUG:
        return

    out = datetime.datetime.now().strftime("#%d.%m.%Y, %H:%M:%S") + '# '
    ...
    if not quiet:
        print_(out, **kwargs)
        if args.logfile and useFile:
            filePrinter(out, **kwargs)
...
def shell():
    ...
    printer('Retrieving speedtest.net configuration...', quiet=quiet)
    try:
        speedtest = Speedtest(
    ...
    if not args.mini:
        printer('Retrieving speedtest.net server list...', quiet=quiet)
        try:
        ...
        if args.server and len(args.server) == 1:
            printer('Retrieving information for the selected server...', quiet=quiet)
        else:
            printer('Selecting best server based on ping...', quiet=quiet)
    ...
    printer('Hosted by %(sponsor)s (%(name)s) [%(d)0.2f km]: '
            '%(latency)s ms' % results.server, quiet=quiet)
    ...
    if args.upload:
        ...
        printer('Skipping upload test', quiet=quiet)
